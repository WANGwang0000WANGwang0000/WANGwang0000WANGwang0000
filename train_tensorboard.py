#  混淆矩阵名称增加了时间戳和ACC filename_2 = f'confusion_matrix_prob_acc_{accuracy}_{current_time}.png'
import argparse
import random
import sys
...
#from models_no_se_stn import get_model
from models_no_se_dot_stn import get_model
...
class Trainer:
    ...
    def run(self):
        ...
        for epoch in range(self.epochs):
            ...
            writer.add_scalars("Accuracy", {"Valid": val_accuracy}, epoch)
        ...
        test_acc, cm, cmp = self.test_model()
    ...
    def test_model(self):
        ...
        filename_1 = f'confusion_matrix_acc_{accuracy}_{current_time}.png'
        path1 = os.path.join('matrix', filename_1)
        # 保存混淆矩阵图像
        plt.savefig(path1)
        # ------------------------------------------------
        ...
        # 将每个单元格的样本数修改为概率
        conf_matrix_prob = conf_matrix / class_counts[:, np.newaxis]
        ...
        plt.figure(figsize=(8, 6))
        sns.set(font_scale=1.2)  # 设置字体大小
        sns.heatmap(conf_matrix_prob, annot=True, fmt=".2f", cmap="Blues", xticklabels=self.classes,
                    yticklabels=self.classes)
        plt.xlabel('Predicted Labels')
        plt.ylabel('True Labels')
        plt.title('Confusion Matrix (fer2013plus)')
        ##----------------------------------
        # 获取当前时间戳
        current_time = datetime.now().strftime("%Y%m%d-%H%M%S")
        ...
        # 构建文件名，包含acc和时间戳
        accuracy = round(accuracy * 100, 6)
        filename_2 = f'confusion_matrix_prob_acc_{accuracy}_{current_time}.png'
        path2 = os.path.join('matrix', filename_2)
        ...
        #     }
        # )
        return temp_acc, conf_matrix, conf_matrix_prob
    ...
    def save(self):
        ...
        # save_path = os.path.join(self.output_directory,f"{self.execution_name}.pt")
...
if __name__ == "__main__":
    ...
    # wandb.init(project="EmoNeXt-X", name=exec_name, anonymous="must")
    ########################
    import os
    ...
    # 拼接日志文件名
    log_name = f"{str(opt.model_size)}_bs_{str(opt.batch_size)}_ep_{str(opt.epochs)}_{current_time}"

    # 创建完整的日志目录路径
    log_dir_path = os.path.join(LOG_DIR, log_name)
    # 创建SummaryWriter实例
